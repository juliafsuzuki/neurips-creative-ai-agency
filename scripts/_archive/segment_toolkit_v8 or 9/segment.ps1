...
param(
    ...
    # Overlap mode: 'tile' means each next clip starts where the previous ended
    # (non-overlapping mosaic, default). 'all' means every beat is a possible
    # start (heavy overlap, useful for exploring alternate framings).
    [ValidateSet('tile','all')]
    [string]$Overlap = 'tile'
)
...
function Cut-Set {
    ...
    # In 'tile' mode, jump the next iteration to that end beat so clips do not
    # overlap. In 'all' mode, iterate every beat (heavy overlap).
    $plan = @()
    $i = 0
    while ($i -lt $beats.Count) {
        $start = $beats[$i]
        if ($start -gt ($duration - 1.0)) { $i++; continue }
        ...
        if ($bestJ -lt 0) { $i++; continue }

        $end = [math]::Min($beats[$bestJ], $duration)
        $dur = $end - $start
        if ($dur -lt $minDur -or $dur -gt $maxDur) {
            $i++
            continue
        }
        ...
        if ($Overlap -eq 'tile') {
            # Next clip starts at the end beat -> no overlap
            $i = $bestJ
        } else {
            $i++
        }
    ...
    foreach ($p in $plan) {
        ...
        $line = "   [{0:D3}] {1,7}s -> {2,7}s  ({3}s)  {4}" -f $idx, $startStr, $endStr, $durStr, $clipName
        Write-Host $line
